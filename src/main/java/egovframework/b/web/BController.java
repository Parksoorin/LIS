...
public class BController {
	...
	@ResponseBody
	public JSONObject qcResultSave(@RequestBody List<Map<String, Object>> list, HttpSession session,
	...
	@ResponseBody
	public JSONObject qcResultfindAll(@RequestBody Map<String, Object> requestMap, HttpSession session,
			HttpServletRequest request, HttpServletResponse response, Model model) throws Exception {
		...
		QcResultRequestDTO qcResultDTO = QcResultRequestDTO.builder().lotNo(String.valueOf(requestMap.get("lotNo")))
				.qcInOut(String.valueOf(requestMap.get("qcInOut")))
				.ruleResult(String.valueOf(requestMap.get("ruleResult")))
				.jundalPart(String.valueOf(requestMap.get("jundalPart"))).jangbi(String.valueOf(requestMap.get("장비명")))
				.level(String.valueOf(requestMap.get("level"))).muljil(String.valueOf(requestMap.get("물질명")))
				.gumsa(String.valueOf(requestMap.get("검사항목"))).dateList(dateList).build();
	...
	public JSONObject login(@RequestParam Map<String, Object> map, HttpSession session, HttpServletRequest request,
		...
		if (id.equals(userBDTO.getId()) && Sha256.encrypt(password).equals(userBDTO.getPassword())) {
